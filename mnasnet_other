        ...
        self.MnasNetBlock = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ('DepthwiseConv',
                     _conv(out_channels * ratio, out_channels * ratio, stride, padding, False, out_channels * ratio)),
...
class MnasNet(nn.Module):
    ...
    def stage(self, stage=1, BlockRepeat=[1, 2], _conv=conv3x3, padding=1, ratio=6):
        ...
        if BlockRepeat[0] == 0:
            modules[name + '_0'] = MnasNetBlock(self.out_channels[stage - 1], self.out_channels[stage], 1, _conv,
                                                padding, ratio, False)
        elif BlockRepeat[0] == 1:
            modules[name + '_0'] = MnasNetBlock(self.out_channels[stage - 1], self.out_channels[stage], 2, _conv,
                                                padding, ratio, False)
        else:
            raise ValueError('BlockRepeat[0] must be 0 or 1')

        for i in range(BlockRepeat[1]):
            modules[name + '_{}'.format(i + 1)] = MnasNetBlock(self.out_channels[stage], self.out_channels[stage], 1,
                                                               _conv, padding, ratio, True)
